...
Future<File> imageAddWaterMark(String imagePath,String textStr) async {
  ...
  var text = textStr;
  // 完整覆盖下的正方形面积
  double rectSize = math.pow(dimension, 2).floorToDouble();
  // 根据面积与字符大小计算文本重复次数
  int textRepeating =
  ((rectSize / math.pow(40, 2) * 2) / (text.length + 8))
      .round(); // text.length + padding 是因为要添加个空格字符

  math.Point<double> pivotPoint = math.Point(dimension / 2, dimension / 2);
  canvas.translate(pivotPoint.x, pivotPoint.y);
  canvas.rotate(-25 * math.pi / 180);
  canvas.translate(
      -pivotPoint.distanceTo(math.Point(0, image.height.toDouble())),
